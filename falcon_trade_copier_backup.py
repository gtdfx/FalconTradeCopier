...
class MT5Manager:
    ...
    def execute_trade(self, symbol, order_type, entry_price, volume, sl=None, tp=None, tps=None, tolerance=2.0):
        ...
        try:
            ...
            if risk_method == 'percent':
                ...
                if not account_info:
                    ...
                    if balance <= 0:
                        ...
                        if sl_distance > 0 and point > 0 and symbol_info.trade_tick_value > 0:
                            ...
                            pip_value = self.get_pip_value(clean_symbol)
                ...
                pip_value = symbol_info.trade_tick_value * pip_multiplier  # Value per pip per lot

                # Calculate lot size
                lot_size = fixed_dollar_risk / (sl_distance * pip_value)
                logger.info(f"Fixed dollar lot size: {lot_size:.2f}")
...
class TelegramPage(QWidget):
    ...
    def setup_ui(self):
        ...
        phone_code_layout.setSpacing(0)
...
class MT5Page(QWidget):
    ...
    def setup_ui(self):
        ...
        # Symbol Prefix and Suffix (side by side in same row)
        symbol_layout = QHBoxLayout()
        symbol_layout.setSpacing(20)
        
        # Prefix section (label + input in same row)
        ...
        self.prefix_input
        symbol_layout.addWidget(prefix_label)
        symbol_layout.addWidget(self.prefix_input)
        
        # Add some spacing between prefix and suffix
        symbol_layout.addSpacing(20)
        
        # Suffix section (label + input in same row)
        ...
        self.suffix_input
        symbol_layout.addWidget(suffix_label)
        symbol_layout.addWidget(self.suffix_input)
        
        # Add spacer to push both fields to the left
        symbol_layout.addStretch()
        
        # Add the combined layout to form
        form_layout.addRow("", symbol_layout)
...
class DashboardPage(QWidget):
    ...
    def update_signal_info(self, signal_data):
        ...
        # Status and error
        self.status_label.setText(signal_data.get("status", "N/A"))
    ...
    def setup_ui(self):
        ...
        # Account info - using grid layout
        account_group = QGroupBox("Account Information")
        account_group.setStyleSheet("QGroupBox { font-size: 14px; font-weight: bold; }")
        account_grid = QGridLayout(account_group)
        account_grid.setHorizontalSpacing(10)
        account_grid.setVerticalSpacing(8)

        # First column
        row = 0
        account_num_label = QLabel("Account Number:")
        account_num_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        account_grid.addWidget(account_num_label, row, 0)
        self.account_num_label = QLabel("Loading...")
        self.account_num_label.setStyleSheet("font-size: 12px;")
        account_grid.addWidget(self.account_num_label, row, 1)
        row += 1

        leverage_label = QLabel("Leverage:")
        leverage_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        account_grid.addWidget(leverage_label, row, 0)
        self.leverage_label = QLabel("Loading...")
        self.leverage_label.setStyleSheet("font-size: 12px;")
        account_grid.addWidget(self.leverage_label, row, 1)
        row += 1

        account_type_label = QLabel("Account Type:")
        account_type_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        account_grid.addWidget(account_type_label, row, 0)
        self.account_type_label = QLabel("Loading...")
        self.account_type_label.setStyleSheet("font-size: 12px;")
        account_grid.addWidget(self.account_type_label, row, 1)
        row += 1

        currency_label = QLabel("Currency:")
        currency_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        account_grid.addWidget(currency_label, row, 0)
        self.currency_label = QLabel("Loading...")
        self.currency_label.setStyleSheet("font-size: 12px;")
        account_grid.addWidget(self.currency_label, row, 1)
        row += 1

        profit_label = QLabel("Profit:")
        profit_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        account_grid.addWidget(profit_label, row, 0)
        self.profit_label = QLabel("Loading...")
        self.profit_label.setStyleSheet("font-size: 12px;")
        account_grid.addWidget(self.profit_label, row, 1)
        row += 1
        ...
        account_grid.addWidget(balance_label, row, 2)
        self.balance_label = QLabel("Loading...")
        self.balance_label.setStyleSheet("font-size: 12px;")
        account_grid.addWidget(self.balance_label, row, 3)
        row += 1

        equity_label = QLabel("Equity:")
        equity_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        account_grid.addWidget(equity_label, row, 2)
        self.equity_label = QLabel("Loading...")
        self.equity_label.setStyleSheet("font-size: 12px;")
        account_grid.addWidget(self.equity_label, row, 3)
        row += 1

        free_margin_label = QLabel("Free Margin:")
        free_margin_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        account_grid.addWidget(free_margin_label, row, 2)
        self.free_margin_label = QLabel("Loading...")
        self.free_margin_label.setStyleSheet("font-size: 12px;")
        account_grid.addWidget(self.free_margin_label, row, 3)
        row += 1

        used_margin_label = QLabel("Used Margin:")
        used_margin_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        account_grid.addWidget(used_margin_label, row, 2)
        self.used_margin_label = QLabel("Loading...")
        self.used_margin_label.setStyleSheet("font-size: 12px;")
        account_grid.addWidget(self.used_margin_label, row, 3)
        row += 1

        margin_level_label = QLabel("Margin Level:")
        margin_level_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        account_grid.addWidget(margin_level_label, row, 2)
        self.margin_level_label = QLabel("Loading...")
        self.margin_level_label.setStyleSheet("font-size: 12px;")
        account_grid.addWidget(self.margin_level_label, row, 3)
        row += 1
        ...
        signal_group.setStyleSheet("QGroupBox { font-size: 14px; font-weight: bold; }")
        ...
        self.symbol_label.setStyleSheet("font-size: 12px;")
        self.type_label = QLabel("-")
        self.type_label.setStyleSheet("font-size: 12px;")
        self.entry_label = QLabel("-")
        self.entry_label.setStyleSheet("font-size: 12px;")
        self.sl_label = QLabel("-")
        self.sl_label.setStyleSheet("font-size: 12px;")
        self.tp_label = QLabel("-")
        self.tp_label.setStyleSheet("font-size: 12px;")
        self.lot_label = QLabel("-")
        self.lot_label.setStyleSheet("font-size: 12px;")
        self.status_label = QLabel("-")
        ...
        channel_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        signal_layout.addRow(channel_label, self.channel_label)
        
        symbol_label = QLabel("Symbol:")
        symbol_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        signal_layout.addRow(symbol_label, self.symbol_label)
        
        type_label = QLabel("Type:")
        type_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        signal_layout.addRow(type_label, self.type_label)
        
        entry_label = QLabel("Entry:")
        entry_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        signal_layout.addRow(entry_label, self.entry_label)
        
        sl_label = QLabel("SL:")
        sl_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        signal_layout.addRow(sl_label, self.sl_label)
        
        tp_label = QLabel("TP:")
        tp_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        signal_layout.addRow(tp_label, self.tp_label)
        
        lot_label = QLabel("Lot Size:")
        lot_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        signal_layout.addRow(lot_label, self.lot_label)
        
        status_label = QLabel("Status:")
        status_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        signal_layout.addRow(status_label, self.status_label)
        
        signal_layout.addRow(self.error_label)
        ...
        performance_title.setStyleSheet("font-size: 16px; font-weight: bold;")
        ...
        # Create simple metric labels
        self.total_pnl_label = QLabel("Total P&L: $0.00")
        self.total_pnl_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        self.win_rate_label = QLabel("Win Rate: 0%")
        self.win_rate_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        self.rr_label = QLabel("Avg R:R: 0.00")
        self.rr_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        self.drawdown_label = QLabel("Max Drawdown: $0.00")
        self.drawdown_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        self.profit_factor_label = QLabel("Profit Factor: 0.00")
        self.profit_factor_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        self.sharpe_label = QLabel("Sharpe Ratio: 0.00")
        self.sharpe_label.setStyleSheet("font-size: 12px; font-weight: bold;")
        ...
        charts_group.setStyleSheet("QGroupBox { font-size: 14px; font-weight: bold; }")
        ...
        self.equity_chart.setStyleSheet("border: 1px solid gray; padding: 10px;")
        
        self.drawdown_chart = QLabel("Drawdown Chart")
        self.drawdown_chart.setMinimumHeight(100)
        self.drawdown_chart.setAlignment(Qt.AlignCenter)
        self.drawdown_chart.setStyleSheet("border: 1px solid gray; padding: 10px;")
        ...
        channel_group = QGroupBox("Channel Performance")
        channel_group.setStyleSheet("QGroupBox { font-size: 14px; font-weight: bold; }")
        ...
        risk_layout.setFieldGrowthPolicy(QFormLayout.ExpandingFieldsGrow)
        risk_layout.setRowWrapPolicy(QFormLayout.DontWrapRows)
        
        # Risk Method
        self.risk_method = QComboBox()
        self.risk_method.addItems(["Fixed Lot", "Percent of Balance", "Fixed Dollar"])
        risk_layout.addRow("Risk Method:", self.risk_method)
        ...
        risk_layout.addRow("Risk %:", self.risk_percent)
        ...
        risk_layout.addRow("Fixed Lot:", self.lot_size)
        ...
        risk_layout.addRow("Fixed $:", self.fixed_dollar)
        ...
        additional_risk_group = QGroupBox("Additional Risk Settings")
        ...
        self.reset_to_default_btn.setStyleSheet("""
            QPushButton {
    ...
    def initialize_performance_data(self):
        ...
        try:
            ...
            if not CHARTING_AVAILABLE:
                logger.warning("Charting libraries not available. Performance tab will be limited.")
                return
    ...
    def update_license_display(self, validation_result, status_report, license_key, machine_id):
        ...
        try:
            ...
            if days_remaining > 30:
                ...
                self.license_status_text.setStyleSheet("color: #198754; font-weight: bold; font-size: 14px;")
            elif days_remaining > 7:
                self.license_status_text.setText("License Expiring Soon")
                self.license_status_text.setStyleSheet("color: #fd7e14; font-weight: bold; font-size: 14px;")
            else:
                self.license_status_text.setText("License Expires Very Soon")
                self.license_status_text.setStyleSheet("color: #dc3545; font-weight: bold; font-size: 14px;")
...
3. Provide your current license key for upgrade pricing
    ...
    def update_connection_status(self, telegram_status=None, mt5_status=None):
        ...
        self.telegram_status.setText("●" if telegram_status else "○"); self.telegram_status.setStyleSheet("color: green;" if telegram_status else "color: red;")
        self.mt5_status.setText("●" if mt5_status else "○"); self.mt5_status.setStyleSheet("color: green;" if mt5_status else "color: red;")
...
class MainWindow(QMainWindow):
    ...
    def apply_theme(self):
        """Apply white background theme with dark text colors"""
